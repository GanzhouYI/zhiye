...
class CommentNet:NSObject {
    static var Model:CommentNet?
    static var predicate:dispatch_once_t = 0
    class func sharedSearch()->CommentNet?{
    ...
    func DownComment(CommentInfo:String,block:NetworkBlock?)
    {
        //测试数据  仅仅用于没有表存在本地的情况下，如果本地创建了表是不行的
        let urlString:String = "http://www.loveinbc.com/zhiye/downComment.php"
        let parameters = ["info":CommentInfo]
        print("DownComment")
        print(parameters)
            ...
            .responseJSON{ response in
                ...
                case .Success:
                    ...
                    else if let JSON = response.result.value as? NSArray
                    {
                       ...
                       CommentManager.shareCommentManager().RefreshComment(infoData)
    ...
    //上传plan到服务器
    func UpLoadComment(planData:[String:String],block:NetworkBlockInfo?)
    {
        let urlString:String = "http://www.loveinbc.com/zhiye/UpLoadComment.php"
    ...
    func DownCommentDataCell(tid:Int,msgItem:CommentItem)
    {
        //测试数据  仅仅用于没有表存在本地的情况下，如果本地创建了表是不行的
        let urlString:String = "http://www.loveinbc.com/zhiye/downCommentCell.php"
        ...
        Alamofire.request(.POST, urlString, parameters: parameters)
            .responseJSON{ response in
                ...
                {
                case .Success:
                    print("DownCommentDataCell 网络连接正常")
                    print(response.result.value!)
                    let str = (response.result.value!)as?String
                    ...
                    if let JSON = response.result.value as? NSArray
                    {
                        ...
                        for i in 0..<JSON.count
                        {
                            ...
                            //
                          //  MySQL.shareMySQL().insertCommentCell(infoDataItem)
                        }
                    }

                    //下载完成
                    //MySQL.shareMySQL().UpdateCommentStatus(String(0), tid: tid)
                    msgItem.status=0
                    break
                case .Failure:
                    print("网络连接错误")
