import UIKit
import Alamofire
class CommentTableDataViewCell:UITableViewCell,UITextViewDelegate
{
    ...
    var msgItem:CommentItem!//总体信息对象
    
    init(frame:CGRect,NOCell:Int,data:CommentItem, reuseIdentifier cellId:String)
    {
    ...
    func AddFriend() {
        if(msgItem.status == 0)
        {
            ...
            addFriend.backgroundColor = UIColor.grayColor()
            addFriend.setTitle("取消关注", forState: UIControlState.Normal)
            MySQL.shareMySQL().addComment(String(LoginModel.sharedLoginModel()!.MyUid!), fuid: String(msgItem.uid))
        }
        else if(msgItem.status == 1)
        {
            msgItem.status = 0
            addFriend.backgroundColor = UIColor(red: 99/255, green: 184/255, blue: 255/255, alpha: 0.5)
            addFriend.setTitle("关注", forState: UIControlState.Normal)
            MySQL.shareMySQL().deleteComment(String(LoginModel.sharedLoginModel()!.MyUid!), fuid: String(msgItem.uid))
        }
    }
